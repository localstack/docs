...
def create_metric_coverage_docs(file_name: str, metrics: dict):
    ...
    <tr>
      ...
      <th>#Tested</th>
      <th>%Tested</th>
      <th>#Tested internally</th>
      <th>%Tested internally</th>
      <th>#aws-validated</th>
      <th>#snapshot-tested</th>
      <th>#snapshot-tested-no-skip</th>
    </tr>
  </thead>
  <tbody>"""
    output += header
    for service in sorted(metrics.keys()):
        ...
        # stats = f"{len(implemented_ops)} out of {len(details)} implemented ({(len(implemented_ops)/len(details)*100):.2f}%)\n\n"
        # stats += f"{all_tested} operations tested ({all_tested/len(implemented_ops)*100:.2f}%)\n -> {internal_tested} operations covered by internal tests ({internal_tested/len(implemented_ops)*100:.2f}%)\n"
        # stats += "<ul>\n"
        # stats += f"<li>{aws_validated_count} operation aws validated\n"
        # stats += f"<ul><li>{snapshot_count} operation snapshot tested ({snapshot_no_skip_count} without skipping any path) </li></ul>\n"
        # stats += "</li>\n"
        # stats += "</ul>\n"
        stats = f"""<tr>
            <td>{service}</td>
            <td>{len(details)}</td>
            <td>{len(implemented_ops)}</td>
            <td>{(len(implemented_ops)/len(details)*100):.2f}%</td>
            <td>{all_tested}</td>
            <td>{all_tested/len(implemented_ops)*100:.2f}%</td>
            <td>{internal_tested}</td>
            <td>{internal_tested/len(implemented_ops)*100:.2f}%</td>
            <td>{aws_validated_count}</td>
            <td>{snapshot_count}</td>
            <td>{snapshot_no_skip_count}</td>
        </tr>"""
        output += stats
    ...
    #         if op_details.get("pro"):
    #             pro_info = " (Pro) "
    #         if op_details.get("aws_validated") or op_details.get("snapshot"):
    #             if not op_details.get('snapshot_skipped_paths', '') and op_details.get("snapshot"):
    #                 aws_validated = " 📸 "
    #             elif op_details.get("snapshot"):
    #                 aws_validated = " 📸* "
    #             else:
    #                 aws_validated = " ✅ "
    #         output += (
    #             "    <tr>\n"
    #             f"      <td>{operation}{pro_info}{tested}{aws_validated}</td>\n"
