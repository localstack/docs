...
By reading this document, you will learn how to use LocalStack to test
your serverless applications powered by Spring Cloud Function framework.

We will create a new Rest API application that will be routing requests
to Cloud Function using `functionRouter` and routing expressions.

The primary language for the application in question would be Kotlin powered by
[Gradle](https://gradle.org) build tool, but described concepts would work for
any other JVM setup.
...
together with LocalStack. It does not cover some of the application-specific topics,
like 404 error handling, parametrized routing, etc., that you need to consider
...
Please, follow instructions from the official [website](https://gradle.org) to
install Gradle build tool on your machine.
...
After initialization, you will find a Gradle wrapper `gradlew`. From now on,
use the wrapper instead of globally installed Gradle:
...
Let's give our project a name, open `settings.gradle`, and adjust the
autogenerated name to something meaningful:

```gradle
rootProject.name = 'localstack-sampleproject'
```

Now we need to define our dependencies. Here's a short overview of what we
will be using in our project.
...
which would also contain all application dependencies. For that purpose, we
use "Shadow Jar" plugin.
...
entry class for our application. You can adjust it for your needs, but don't
...
it for AWS Lambda. In this project, we are following
...
requests to different `Beans` based on predefined routing expressions. Let's
configure it to lookup our function Beans by HTTP method and path, create a
...
Once configured, you can use FunctionInvoker as a handler for your Rest API lambda
function. It will automatically pick up the configuration we have just set.
```java
...
In our sample project we are using JSON format for reqeusts/responses. The easiest
way to get started with JSON is to use the Jackson library. Let's configure it
by creating a new configuration class `JacksonConfiguration.kt` under
...
* to parse request payload using ObjectMapper. Note that ObjectMapper
does not necessarily need to be a JSON only. It could also be XML or any other
Mapper extended from standard ObjectMapper. Your application may even support
multiple protocols with different request/response formats at once.
...
And now utility function to process API Gateway requests:
...
We know Java's cold start is always a pain. To minimize this pain, we will try
to define a pre-warming endpoint within the Rest API. By invoking this function
every 5-10 mins we can make sure Rest API lambda is always kept in a pre-warmed
state.
...
Now you can add a scheduled event to the Rest API lambda function with the following
...
HTTP requests are not the only thing our Spring Function-powered
lambdas can do. We can still define pure lambda functions, DynamoDB stream
handlers, and so on.

Below you can find a little example of few lambda functions grouped in
`LambdaApi` class.
...
You may have noticed we used `DynamodbEvent` in the last example. The `Lambda-Events`
package comes with a set of predefined wrappers that you can use to handle
different lifecycle events from AWS.
