...
$ LAYER_ARN=$(awslocal lambda publish-layer-version --layer-name layer1 --zip-file file:///tmp/testlayer.zip | jq -r .LayerArn)
{{< / command >}}
...
$ awslocal lambda create-function --function-name func1 --runtime python3.8 --role r1 --handler testlambda.handler --timeout 30 --zip-file file:///tmp/testlambda.zip --layers $LAYER_ARN:1
{{< / command >}}
