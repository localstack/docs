...
{{< callout "note" >}}
LocalStack cannot emulate or run standard Amazon Machine Images (AMIs) because AWS does not allow them to be downloaded or used outside its environment. However, you can use [Packer](https://packer.io/) or a similar tool to modify the build process for Docker, which is compatible with LocalStack.
{{< /callout >}}
